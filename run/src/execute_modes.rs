...
use crate::{Arguments, load_data, load_data_and_init_sim};
...
pub async fn download(arguments: Arguments) -> anyhow::Result<()> {
    ...
    if !arguments.allow_downloads {
        return Err(MissingArguments("Cannot download tables without the download flag enabled!".to_string())).context("Mode: Downloading tables");
    }
    CensusData::load_all_tables_async(arguments.data_directory, arguments.area_code, arguments.allow_downloads)
        .await
...
pub async fn execute_arguments(arguments: Arguments) -> anyhow::Result<()> {
    match arguments.mode {
        ...
        SimMode::Render => { unimplemented!("Cannot use renderer on current Rust version (2018") }

...
impl SimulatorBuilder {
    ...
    pub fn assign_buildings_to_output_areas(
        ...
        let to_delete: Vec<usize> = output_areas
            ...
            .filter_map(|(index, area)| {
                if !possible_buildings_per_area.contains_key(area.id().code()) {
                    Some(index)
                } else {
                    None
                }
    ...
    pub fn generate_citizens(
        ...
        if no_buildings > 0 {
            warn!(
                "Failed to generate households for {} Output Areas, as no buildings exist!",
                no_buildings
            );
        }
        Ok(())
    ...
    pub fn build_schools(&mut self) -> anyhow::Result<()> {
        ...
        let (mut output_area_citizens, mut output_area_buildings, output_area_ids) = self
            ...
            .map(|area| {
                (
                    &mut area.citizens,
                    &mut area.buildings,
                    &mut area.output_area_id,
                )
            })
            .fold(
                (Vec::new(), Vec::new(), Vec::new()),
                |(mut accum_citizens, mut accum_buildings, mut accum_ids),
                 (citizens, buildings, id)| {
        ...
        let (students, teachers): (Vec<Vec<&mut Citizen>>, Vec<&mut Citizen>) =
            output_area_citizens
                ...
                .reduce(
                    || (Vec::new(), Vec::new()),
                    |(mut accum_students, mut accum_teachers), (item_students, item_teachers)| {
        ...
        debug!(
            "There are {} age groups, with {} students and {} teachers",
            students.len(),
            students
                .iter()
                .map(|age_group| age_group.len())
                .sum::<usize>(),
            teachers.len()
        );
        ...
        if crate::config::CREATE_DEBUG_DUMPS {
            let school_boundaries: Vec<(Point<i32>, Vec<Coordinate<i32>>)> = building_locations
                .iter()
                .filter_map(|building| {
            ...
            let file = File::create(filename.clone())
                .context(format!("Failed to create file: '{}'", filename.clone()))?;
            serde_json::to_writer(file, &school_boundaries)
                .context("Failed to dump school boundaries to file!")?;
        }
        ...
        let finding_closest_school =
            |citizen: &Citizen, get_multiple: bool| -> Result<Vec<&RawBuilding>, SimError> {
                ...
                let buildings = output_area_buildings
                    .get(area_code.index())
                    .ok_or_else(|| SimError::InitializationError {
                        message: format!("Couldn't retrieve output area: {}", area_code),
                    })?;

                let building = buildings
                    .get(citizen.household_code.building_index())
                    .ok_or_else(|| SimError::InitializationError {
                        message: format!(
                            "Couldn't retrieve household home: {}",
                            citizen.household_code
                        ),
                    })?;
                Ok({
                    let closest_schools_index =
                        school_lookup.find_seeds_for_point(building.get_location())?;
                    closest_schools_index
                        .into_iter()
                        .filter_map(|index| {
        ...
        info!(
            "Assigned Students to {} raw schools",
            citizens_per_raw_school.len()
        );
        ...
        if crate::config::CREATE_DEBUG_DUMPS {
            ...
            let school_boundaries: Vec<(Point<i32>, Vec<Coordinate<i32>>)> =
                citizens_per_raw_school
                    .iter()
                    .filter_map(|(_, (_, _, building))| {
                        let path = all_boundaries.get(&building.boundary_id())?;
                        Some((building.center(), path.exterior().clone().0))
                    })
                    .collect();
            ...
            let debug_school_data: HashMap<String, (Vec<CitizenID>, Vec<CitizenID>)> =
                citizens_per_raw_school
                    .iter()
                    .map(|(school, (students, teachers, _building))| {
                        return (
                            format!("{:?}", school.x_y()),
                            (
                                students
                                    .iter()
                                    .map(|age_group| age_group.iter().map(|student| student.id()))
                                    .flatten()
                                    .collect(),
                                teachers.iter().map(|teacher| teacher.id()).collect(),
                            ),
                        );
                    })
                    .collect();
        ...
        warn!(
            "Failed to assign schools to {} teachers",
            failed_teacher_count
        );
        ...
        citizens_per_raw_school.into_iter().for_each(
            |(_school_position, (students, teachers, building))| {
                ...
                let student_ids = students
                    .iter()
                    .map(|age_group| age_group.iter().map(|student| student.id()).collect())
                    .collect();
                ...
                debug_stats.insert(
                    format!(
                        "({},{}",
                        school.get_location().x(),
                        school.get_location().y()
                    ),
                    stats,
                );
                class_total += school.classes().len();
                offices_total += school.offices().len();
                misc_staff_total += school
                    .offices()
                    .iter()
                    .map(|office| office.len())
                    .sum::<usize>();
                schools_total += 1;
    ...
    pub fn build_workplaces(
        ...
        debug!(
            ...
            citizens_to_allocate.len()
        );
        debug!(
            "{} Citizens have not been assigned a workplace area!",
            ...
            .zip(output_area_citizens)
        {
            ...
            let workplace_output_area_buildings =
                match output_area_buildings.get_mut(workplace_area_index) {
                    Some(workplace) => workplace,
                    None => {
                        error!(
                            "Workplace output area {} doesn't exist!",
                            workplace_area_index
                        );
            ...
            let possible_buildings =
                match possible_buildings_per_area.get_mut(workplace_output_id.code()) {
                    ...
                    None => {
                        error!(
                            "No Workplace buildings exist for Output Area: {}",
                            workplace_output_id
                        );
                        continue 'citizen_allocation_loop;
                    }
                };
            match SimulatorBuilder::assign_buildings_per_output_area(
                workplace_output_id.clone().clone(),
                citizens,
                possible_buildings,
                ...
                Err(e) => {
                    error!(
                        "Failed to assign buildings for Output Area: {}, Error: {}",
                        workplace_output_id, e
                    );
                }
            }
        }
    ...
    fn assign_buildings_per_output_area(
        ...
        if citizen_ids.len() == 0 {
            warn!(
                "No buildings can be assigned to area {} as no workers exist: {:?}",
                workplace_area_code, citizen_ids
            );
            return Ok(Vec::new());
        }
        if possible_buildings.len() == 0 {
            warn!(
                "No buildings can be assigned to area {} as no buildings exist: {:?}",
                workplace_area_code, possible_buildings
            );
            return Ok(Vec::new());
        }
        ...
        let mut citizen_ids_per_occupation: EnumMap<OccupationType, Vec<&mut Citizen>> =
            citizen_ids
                .into_iter()
                .filter_map(|citizen| Some((citizen.detailed_occupation()?, citizen)))
        ...
        let mut building_per_occupation: EnumMap<OccupationType, (usize, Vec<RawBuilding>)> =
            EnumMap::default(); //from_array(*(vec![(0, Vec::with_capacity(1000));OccupationType::iter().len()].as_slice()));
        ...
        required_space_per_occupation
            .iter()
            .for_each(|(occupation, size)| {
                differences[occupation] = *size as isize;
            });
        ...
        for occupation in OccupationType::iter() {
            ...
            match SimulatorBuilder::assign_workplaces_to_citizens_per_occupation(
    ...
    /// `next_building_index` is the index to start assigning indexes to new buildings
    fn assign_workplaces_to_citizens_per_occupation(
        workplace_area_code: OutputAreaID,
        ...
        for (index, citizen) in citizens.iter_mut().enumerate() {
            assert_eq!(
                citizen.detailed_occupation().unwrap(),
                occupation,
                "Citizen does not have the specified occupation!"
            );
            // 2 Cases
            ...
            current_workplace = match current_workplace.add_citizen(citizen.id()) {
                Ok(_) => current_workplace,
                Err(_) => {
                    workplace_buildings.push(Box::new(current_workplace));
                    ...
                    new_workplace
                        ...
                        .context("Cannot add Citizen to freshly generated Workplace!")?;
                    new_workplace
                }
            };
    ...
    pub fn build(&mut self) -> anyhow::Result<()> {
        ...
        timer.code_block_finished(&format!("Built schools", ))?;

        // TODO Currently any buildings remaining are treated as Workplaces
        let possible_workplaces: HashMap<String, Vec<RawBuilding>> = possible_buildings_per_area
            .drain()
            .filter_map(|(area, mut classified_buildings)| {
                let buildings: Vec<RawBuilding> = classified_buildings
                    .drain()
                    .filter(|(class, _)| {
                        *class != TagClassifiedBuilding::School
                            && *class != TagClassifiedBuilding::Household
                    })
                    .flat_map(|(_, a)| a)
                    .collect();
        ...
        info!(
            "Starting to build workplaces for {} areas",
            self.output_areas.len()
        );
...
fn get_area_code_for_raw_building(
    ...
    if let Some(boundary) = boundary {
        ...
        warn!(
            "Raw Building is missing Boundary with id: {:?}",
            building.boundary_id()
        );
    }
    None
}
